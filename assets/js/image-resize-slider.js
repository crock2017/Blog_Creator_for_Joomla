...
(function() {
	...
	var ImageResizeSlider = exports.ImageResizeSlider = function() {
		    function e(t) {
            var a = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
            _classCallCheck(this, e);
            this.quill = t;
	
			this.quill.theme.tooltip.root.addEventListener('input', evt => {
			if (evt.target.className === 'ql-image') {
				//-- inserted image
			}
		});
			var IRS,recentWidth;
			this.quill.root.addEventListener('click', evt => {
				var cancel = new CustomEvent (
						'slider',
						{
							detail: t.root,
							bubbles: true,
				...
				if(evt.toElement.nodeName === 'IMG') {
					...
					slider.setAttribute('id', 'blg_resize_slider');
					slider.style = 'position:absolute; bottom: -20px;';
					slider.setAttribute('data-rs-min', '10');
					slider.setAttribute('data-rs-max', '1010');
					slider.setAttribute('data-rs-steps', '100');
					this.quill.theme.tooltip.root.appendChild(slider);
					recentWidth = evt.target.getAttribute('width');
					
						var config = {
							 onRangeChange:function(slider){
								 var value = slider.getRange();
								 evt.target.setAttribute('width', value);
							//-export events into imageresize	 
								t.root.dispatchEvent(cancel);
								t.root.dispatchEvent(event);
							 },
							afterInit:function(slider){
							slider.setRange(recentWidth);
						}
						 };
					//--create slider
						IRS = RangeSlider.create('#blg_resize_slider', config);
					
				this.quill.on('text-change', function(delta, oldDelta, source) {
				var _attribute = delta.ops.find(function(element){
							 return element.attributes; 
						  });
					if(typeof(_attribute) !== 'undefined' && _attribute.attributes.width && _attribute.attributes.width.length > 0) {
						widthFromResize = _attribute.attributes.width;
					//-set new range for a slider
						IRS.setRange(widthFromResize);
					}
					});
				} else {
					//-close all
					t.root.dispatchEvent(cancel);
					if( document.getElementById('blg_resize_slider')) document.getElementById('blg_resize_slider').remove();
				}
			});
//-- Slider---Thanks to Salis Braimah: https://github.com/msbrime----------------------------------
    ...
    function createSpan(spanElem, classes, spanText) {
        ...
        spanElem.firstChild
            .appendChild(document.createTextNode(spanText + 'px'));
        ...
        for (var i = 1; i <= slider.size; i++) {
            ...
            switch (true) {
                ...
                default:
                    rangeSpan =
                    createSpan(rangeSpan, aSpanClasses,
                        String(Math.ceil(slider.config.min + (i * stepValue))));
							   
							//String(Math.ceil((n+1)/10)*10));  
                    break;
            }
...
Quill.register('modules/imageResizeSlider', exports.ImageResizeSlider);
